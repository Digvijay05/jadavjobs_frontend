...
// function JOBTable() {
//     const name = "lOVcGUoRxcEcfyvFMAnDgaF1VWw"
//     const linklist = [];
...
//                     lname.push(data.jobs[i].name);
//                     link.push(data.jobs[i].link);
//                 }
//                 const linklist = [];
//                 createTable();
//                 refresh();
...
//         console.log("function", lname, link);
//         if (lname.length == 0) {
//             for (var i = 0; i < 10; i++) {
//                 console.log("started!", lname.length)
//                 linklist[i] = <TableCard link={!link[i] ? "loading..." : `${link[i]}`} name={!lname[i] ? "loading..." : `${lname[i]}`} />
//             }
//         } else {
...
//                     <a type="url" target="_blank" href={"http://localhost:8000" + props.link} className="text-sm bg-dodger-blue font-medium text-sky-800 hover:text-blue-800" onClick={() => {
//                         console.log(props)
//                     }}>{props.name}
//                     </a>
//                 </div>
...
function App() {
    const blog = [];
    for (let i = 0; i < 2; i++) {
        blog[i] = < JOBTable tableName={"Table " + i} />;
    }
    const content = (<section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
            <div className="flex flex-wrap -m-4">{blog}</div>
        </div>
    </section>)
    return (
        <div className="App">
